...
class DistributionAnalyzer:
    ...
    def format_table_health_report(self, table_dist: TableDistribution) -> None:
        ...
        rprint(f"• Node Coverage: {len(table_nodes)}/{len(cluster_nodes)} nodes ({len(table_nodes)/len(cluster_nodes)*100:.0f}%)")
        ...
        if shard_cv > 0.3:
            rprint(f"• [red]⚠  Shard Imbalance:[/red] Range {min_shards}-{max_shards} shards per node (CV: {shard_cv:.2f})")
        else:
            rprint(f"• [green]✓ Shard Balance:[/green] Well distributed (CV: {shard_cv:.2f})")
        
        # Storage distribution analysis  
        if storage_cv > 0.4:
            rprint(f"• [red]⚠  Storage Imbalance:[/red] Range {format_storage_size(min_storage)}-{format_storage_size(max_storage)} per node (CV: {storage_cv:.2f})")
        else:
        ...
        try:
            ...
            for node_name, node_data in table_dist.node_distributions.items():
                ...
                if node_info and hasattr(node_info, 'attributes') and node_info.attributes and 'zone' in node_info.attributes:
            ...
            if zone_distribution:
                ...
                for zone in sorted(zone_distribution.keys()):
                    ...
                    rprint(f"• {zone}: {zone_data['nodes']} nodes, {zone_data['shards']} shards, {format_storage_size(zone_data['size'])}")
    ...
    def detect_shard_count_imbalance(self, table: TableDistribution) -> Optional[DistributionAnomaly]:
        ...
        if max_shards - min_shards > 1:
            overloaded_nodes = [
                node for node, data in table.node_distributions.items()
                if data['total_shards'] == max_shards
            ]
            underloaded_nodes = [
                node for node, data in table.node_distributions.items()
                if data['total_shards'] == min_shards
            ]
            
            if overloaded_nodes and underloaded_nodes:
                recommendations.append(
                    f"Move shards from {overloaded_nodes[0]} to {underloaded_nodes[0]}"
                )
    ...
    def detect_node_coverage_issues(self, table: TableDistribution) -> Optional[DistributionAnomaly]:
        ...
        recommendations = [
            f"Consider adding replicas to nodes: {', '.join(sorted(nodes_without_shards))}"
        ]
    ...
    def detect_document_imbalance(self, table: TableDistribution) -> Optional[DistributionAnomaly]:
        ...
        # Generate recommendations
        recommendations = [
            "Document imbalance may indicate data skew - consider reviewing shard routing"
        ]
        ...
        if max_docs > min_docs * 3:  # If difference is > 3x
            recommendations.append(
                f"Significant document skew detected ({min_docs:,} to {max_docs:,} docs per node)"
            )
    ...
    def format_distribution_report(self, anomalies: List[DistributionAnomaly], tables_analyzed: int) -> None:
        ...
        if not anomalies:
            rprint(f"[green]✓ No significant shard distribution anomalies detected in top {tables_analyzed} tables![/green]")
            return
        
        # Show analysis scope
        unique_tables = set(anomaly.table.full_table_name for anomaly in anomalies)
        rprint(f"[blue]📋 Analyzed {tables_analyzed} largest tables, found issues in {len(unique_tables)} tables[/blue]")
        rprint()
            ...
            for i, anomaly in enumerate(anomalies[:5], 1):  # Top 5 recommendations
                ...
                if anomaly.anomaly_type == "Shard Count Imbalance":
                    if 'shard_counts' in anomaly.details:
                        ...
                        rprint(f"   [red]⚠  Issue:[/red] {overloaded[0]} has {max_count} shards while {underloaded[0]} has only {min_count} shards")
                    ...
                    if 'storage_sizes' in anomaly.details:
                        ...
                        rprint(f"   [red]⚠  Issue:[/red] Storage ranges from {format_storage_size(min_size)} ({underloaded}) to {format_storage_size(max_size)} ({overloaded}) - {max_size/min_size:.1f}x difference")
                    ...
                    if 'nodes_without_shards' in anomaly.details:
                        ...
                        rprint(f"   [red]⚠  Issue:[/red] Table missing from {len(missing_nodes)} nodes ({coverage_ratio:.0%} cluster coverage)")
                        rprint(f"   [dim]   Missing from: {', '.join(missing_nodes[:3])}{'...' if len(missing_nodes) > 3 else ''}[/dim]")
                    ...
                    if 'document_counts' in anomaly.details:
                        ...
                        rprint(f"   [red]⚠  Issue:[/red] Document counts range from {min_docs:,} to {max_docs:,} ({ratio:.1f}x difference)")
                
                # Show recommendations
                rprint(f"   [green]💡 Solutions:[/green]")
        ...
        rprint(f"[dim]• Warning issues (score 25-50): {len([a for a in anomalies if 25 <= a.combined_score < 50])}[/dim]")